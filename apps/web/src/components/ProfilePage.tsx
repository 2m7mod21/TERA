...
export default function ProfilePageClient({ data }: { data: any }) {
  ...
  const [liveAvatarUrl, setLiveAvatarUrl] = useState<string>(profile.avatarUrl ?? "");
  const [liveCoverUrl, setLiveCoverUrl] = useState<string>(profile.coverUrl ?? "");
  ...
  const handleFileUpload = async (
    ...
    if (!file) return;
    const isAvatar = key === "avatarUrl";
    setUploading(prev => ({ ...prev, [isAvatar ? "avatar" : "cover"]: true }));
    try {
      ...
      if (d.success && d.url) {
        ...
        setEditData(prev => ({ ...prev, [key]: d.url }));
        // Optimistically update visible image immediately
        if (isAvatar) setLiveAvatarUrl(d.url);
        else setLiveCoverUrl(d.url);
        // Auto-save directly to DB when uploaded from profile header icons
        if (autoSave) {
          await updateProfile({ ...editData, [key]: d.url });
          router.refresh();
        }
      }
    } catch { /* ignore */ } finally {
      setUploading(prev => ({ ...prev, [isAvatar ? "avatar" : "cover"]: false }));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 mobile-pb-nav">
      ...
      <div className="relative h-40 sm:h-64 md:h-80 w-full bg-gradient-to-br from-violet-900 via-purple-900 to-zinc-900 overflow-hidden">
        {liveCoverUrl
          ? <img src={liveCoverUrl} alt="Cover" className="w-full h-full object-cover opacity-80" loading="lazy" decoding="async" />
        ...
        {isOwnProfile && (
          <label className="absolute top-4 right-4 glass px-3 py-2 rounded-xl text-sm text-zinc-300 hover:text-white transition-colors cursor-pointer flex items-center gap-2">
            ...
            <input type="file" accept="image/*" className="hidden" disabled={uploading.cover} onChange={e => handleFileUpload(e, "coverUrl", true)} />
          </label>
        )}
      </div>
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-end gap-3 -mt-12 sm:-mt-20 pb-6 border-b border-zinc-800">
          ...
          <div className="relative flex-shrink-0">
            <div className="w-24 h-24 sm:w-36 sm:h-36 rounded-full border-4 border-zinc-950 bg-gradient-to-br from-violet-500 to-pink-500 overflow-hidden shadow-2xl">
              {liveAvatarUrl
                ? <img src={liveAvatarUrl} alt={profile.displayName} className="w-full h-full object-cover" loading="eager" />
                : profile.avatarUrl
            ...
            {isOwnProfile && (
              <label className={`absolute bottom-1 right-1 w-8 h-8 bg-zinc-800 border border-zinc-700 rounded-full flex items-center justify-center hover:bg-violet-600 transition-colors cursor-pointer ${uploading.avatar ? "opacity-60 cursor-wait" : ""}`}>
                {uploading.avatar
                  ? <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  : <Camera className="w-4 h-4" />}
                <input type="file" accept="image/*" className="hidden" disabled={uploading.avatar} onChange={e => handleFileUpload(e, "avatarUrl", true)} />
              </label>
            )}
          </div>
